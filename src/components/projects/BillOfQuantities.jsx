...
export default function BillOfQuantities({ quoteLines, projectId, project, quote, quoteId, onUpdateQuoteLine, progressEntries, collectionTasks }) {
    ...
    const renderInvoiceTotals = (invoiceNum) => {
        ...
        return (
            <div className="mt-6 p-4 bg-white border border-gray-200 rounded-lg flex justify-end" dir="rtl">
                        ...
                        {invoiceNumbers.map(invoiceNum => (
                            <TabsContent key={invoiceNum} value={`invoice-${invoiceNum}`}>
                                <div className="overflow-x-auto" dir="rtl">
                                    <Table>
                                        <TableHeader><TableRow><TableHead className="text-right w-20">סעיף</TableHead><TableHead className="text-right w-64">תיאור הפריט</TableHead><TableHead className="text-right w-28">כמות</TableHead><TableHead className="text-right w-32">מחיר יחידה</TableHead><TableHead className="text-right w-32">סה״כ</TableHead><TableHead className="text-right w-24">כמות מצטברת</TableHead><TableHead className="text-right w-32">ביצוע מצטבר</TableHead><TableHead className="text-right w-40">הערות</TableHead><TableHead className="text-right">חיוב {invoiceNum}</TableHead></TableRow></TableHeader>
                                        <TableBody>
                                            {localQuoteLines && localQuoteLines.map((line) => {
                                                if (line.is_header) {
                                                    return (<TableRow key={line.id} className="bg-[rgba(96,165,250,0.1)]"><TableCell colSpan={9} className="text-right font-bold text-blue-900 text-lg py-3">{line.clause_number && <span className="text-gray-500 ml-2">{line.clause_number}</span>}{line.name_snapshot || line.model_snapshot}</TableCell></TableRow>);
                                                }
                                                const entries = invoicesData[line.id] || [];
                                                ...
                                                return (
                                                    <TableRow key={line.id}>
                                                        <TableCell className="text-right"><Input type="text" value={line.clause_number || ''} onChange={(e) => handleClauseNumberChange(line.id, e.target.value)} onBlur={(e) => handleClauseNumberBlur(line.id, e.target.value)} className="w-20 text-center border-gray-300" /></TableCell>
                                                        <TableCell className="text-right"><div><p className="font-medium text-slate-800">{line.name_snapshot}</p>{line.description_snapshot && (<p className="text-sm text-gray-500">{line.description_snapshot}</p>)}</div></TableCell>
                                                        <TableCell className="text-right"><Input type="number" min="0" step="0.01" value={line.quantity || 0} onChange={(e) => { const newQuantity = parseFloat(e.target.value) || 0; const newLineTotal = newQuantity * (line.price_no_vat_snapshot || 0); setLocalQuoteLines(prev => prev.map(l => l.id === line.id ? { ...l, quantity: newQuantity, line_total: newLineTotal } : l)); }} onBlur={async () => { try { const newLineTotal = line.quantity * (line.price_no_vat_snapshot || 0); await QuoteLine.update(line.id, { quantity: line.quantity, line_total: newLineTotal }); toast.success("כמות עודכנה"); if (onUpdateQuoteLine) onUpdateQuoteLine(line.id, true); } catch (error) { console.error("Failed to update quantity:", error); toast.error("שגיאה בעדכון כמות"); } }} className="w-24 text-center border-gray-300" /></TableCell>
                                                        <TableCell className="text-right"><Input type="number" min="0" step="0.01" value={line.price_no_vat_snapshot || 0} onChange={(e) => { const newPrice = parseFloat(e.target.value) || 0; const newLineTotal = (line.quantity || 0) * newPrice; setLocalQuoteLines(prev => prev.map(l => l.id === line.id ? { ...l, price_no_vat_snapshot: newPrice, line_total: newLineTotal } : l)); }} onBlur={async () => { try { const newLineTotal = (line.quantity || 0) * (line.price_no_vat_snapshot || 0); await QuoteLine.update(line.id, { price_no_vat_snapshot: line.price_no_vat_snapshot, line_total: newLineTotal }); toast.success("מחיר יחידה עודכן"); if (onUpdateQuoteLine) onUpdateQuoteLine(line.id, true); } catch (error) { console.error("Failed to update price:", error); toast.error("שגיאה בעדכון מחיר"); } }} className="w-28 text-center border-gray-300" /></TableCell>
                        ...
                        <TabsContent key="summary-content" value="summary">
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {invoiceNumbers.map(num => {
                                    ...
                                    return (<Card key={num} className="p-4 bg-white border border-gray-200 shadow-sm"><h4 className="font-semibold mb-2 text-gray-800">חשבון {num}</h4><p className="text-2xl font-bold text-blue-600">₪{finalTotalForCard.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p><p className="text-sm text-gray-500">כולל מע״מ</p></Card>);
                                })}
                            </div>
                            <Card className="mt-6 p-6 bg-white border border-gray-200 shadow-sm">
                                <h3 className="text-xl font-bold mb-4 text-slate-800">סיכום כללי</h3>
                                <div className="grid grid-cols-3 gap-6 text-center">
                                    <div><p className="text-sm text-gray-500">סך הכל חויב</p><p className="text-3xl font-bold text-green-700">₪{invoiceNumbers.reduce((sum, num) => { const subtotal = calculateInvoiceTotals(num); const { deduction_insurance_percentage = 0, deduction_retention_percentage = 0, deduction_lab_tests_percentage = 0 } = project || {}; const ins = subtotal * (deduction_insurance_percentage / 100); const ret = subtotal * (deduction_retention_percentage / 100); const lab = subtotal * (deduction_lab_tests_percentage / 100); const td = ins + ret + lab; const tad = subtotal - td; return sum + (tad * 1.18); }, 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p></div>
                                    <div><p className="text-sm text-gray-500">סך הכל פרויקט</p><p className="text-3xl font-bold text-slate-800">₪{(quote?.total || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p></div>
                                    <div><p className="text-sm text-gray-500">נותר לחיוב</p><p className="text-3xl font-bold text-orange-600">₪{((quote?.total || 0) - invoiceNumbers.reduce((sum, num) => { const subtotal = calculateInvoiceTotals(num); const { deduction_insurance_percentage = 0, deduction_retention_percentage = 0, deduction_lab_tests_percentage = 0 } = project || {}; const ins = subtotal * (deduction_insurance_percentage / 100); const ret = subtotal * (deduction_retention_percentage / 100); const lab = subtotal * (deduction_lab_tests_percentage / 100); const td = ins + ret + lab; const tad = subtotal - td; return sum + (tad * 1.18); }, 0)).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p></div>
                                </div>
                            </Card>
                        </TabsContent>
                    </Tabs>
