...
export default function QuotePreview({ quote, quoteLines, lead }) {
    ...
    return (
        <div className="bg-white" style={{ width: '794px', maxWidth: '794px', margin: '0 auto' }} dir="rtl">
            ...
            <div id="pdf-sect-header" className="p-8 border-b-2 border-blue-600 flex justify-between items-center">
                <div className="text-right">
                    ...
                    <p className="text-blue-700 font-medium">מיזוג אוויר | חימום תת רצפתי | אוורור ופינוי עשן</p>
                    <p className="text-slate-600 text-sm mt-2">ח.פ: 516524287 | מספר קבלן: 37992</p>
                </div>
                <img
            ...
            <div id="pdf-sect-details" className="p-6 grid grid-cols-2 gap-4 bg-slate-50">
                <div className="text-right">
                ...
                <div className="text-left">
                    <h2 className="text-2xl font-bold text-slate-900 mb-2">הצעת מחיר</h2>
                    <p className="text-slate-600">תאריך: {format(new Date(), 'dd/MM/yyyy', { locale: he })}</p>
                    <p className="text-slate-600">מספר הצעה: {quote?.quote_number || `Q-${Date.now().toString().slice(-8)}`}</p>
                    <p className="text-slate-600 mt-2">תוקף: 30 יום</p>
                </div>
            </div>
            ...
            <div id="pdf-sect-table" className="px-6 pt-6 pb-0">
                ...
                <div className="overflow-hidden border border-slate-300 rounded-lg">
                    <table className="w-full" style={{ borderCollapse: 'collapse' }}>
                        <thead className="bg-blue-600 text-white" style={{ display: 'table-header-group' }}>
                            <tr>
                        ...
                        <tbody>
                            {quoteLines.map((line, index) => {
                                if (line.is_header) {
                                    return (
                                        <tr key={line.id || index} className="bg-blue-50"
                                            style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                                            <td colSpan={7} className="p-3 text-right font-bold text-blue-900 text-lg">
                                                {line.clause_number && <span className="text-slate-600 ml-2">{line.clause_number}</span>}
                                ...
                                return (
                                    <tr key={line.id || index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}
                                        style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                                        <td className="p-3 text-right text-sm font-medium text-slate-700">{line.clause_number || '-'}</td>
                                        <td className="p-3 text-right text-sm text-slate-600">{line.category_snapshot}</td>
                                        <td className="p-3 text-right font-medium text-blue-900">
                                            {line.model_snapshot || line.name_snapshot}
                                            {line.sub_category_snapshot && (
                                                <span className="block text-xs text-slate-500 font-normal">{line.sub_category_snapshot}</span>
                                            )}
                                        </td>
                                        <td className="p-3 text-center text-sm font-medium text-slate-700">
                                        ...
                                        <td className="p-3 text-right font-bold text-blue-600">₪{line.line_total?.toLocaleString() || 0}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
            ...
            <div id="pdf-sect-summary" className="px-6 py-6 flex justify-end"
                style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                <div className="w-80 bg-slate-50 border rounded-lg p-4">
                    <div className="space-y-3">
                        ...
                        {discountPercentage > 0 && (
                            <div className="flex justify-between py-2 text-slate-700 font-semibold border-t">
            ...
            <div id="pdf-sect-terms" className="p-6 bg-slate-50 border-t"
                style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                ...
                {quote?.notes && (
                    <div className="mt-4 p-4 bg-yellow-50 border-r-4 border-yellow-400">
            ...
            <div id="pdf-sect-footer" className="p-6 border-t bg-blue-600 text-white text-center"
                style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
