...
export default function QuotePreview({ quote, quoteLines, lead }) {
    ...
    return (
        <div className="bg-white" style={{ width: '794px', maxWidth: '794px', margin: '0 auto' }} dir="rtl">
            ...
            <div id="pdf-sect-details" className="p-6 grid grid-cols-2 gap-4 bg-white">
                <div className="text-right">
                ...
                <div className="text-left">
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">הצעת מחיר</h2>
                    <p className="text-gray-500">תאריך: {format(new Date(), 'dd/MM/yyyy', { locale: he })}</p>
                    <p className="text-gray-500">מספר הצעה: {quote?.quote_number || `Q-${Date.now().toString().slice(-8)}`}</p>
                    <p className="text-gray-500 mt-2">תוקף: 30 יום</p>
                </div>
            </div>
                    ...
                    <table className="w-full" style={{ borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            {quoteLines.map((line, index) => {
                                    ...
                                    return (
                                        <tr key={line.id || index} className="bg-blue-50"
                                            ...
                                            <td colSpan={7} className="p-3 text-right font-bold text-blue-900 text-lg">
                                                {line.clause_number && <span className="text-gray-500 ml-2">{line.clause_number}</span>}
                                ...
                                return (
                                    <tr key={line.id || index} className={index % 2 === 0 ? 'bg-white' : 'bg-white'}
                                        style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                                        <td className="p-3 text-right text-sm font-medium text-gray-700">{line.clause_number || '-'}</td>
                                        <td className="p-3 text-right text-sm text-gray-500">{line.category_snapshot}</td>
                                        <td className="p-3 text-right font-medium text-blue-900">
                                            {line.model_snapshot || line.name_snapshot}
                                            {line.sub_category_snapshot && (
                                                <span className="block text-xs text-gray-500 font-normal">{line.sub_category_snapshot}</span>
                                            )}
                                        </td>
                                        ...
                                        <td className="p-3 text-right font-bold text-[#B8922E]">₪{line.line_total?.toLocaleString() || 0}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
            ...
            <div id="pdf-sect-summary" className="px-6 py-6 flex justify-end"
                ...
                <div className="w-80 bg-white border rounded-lg p-4">
                    <div className="space-y-3">
                        ...
                        {discountPercentage > 0 && (
                            <div className="flex justify-between py-2 text-gray-700 font-semibold border-t">
            ...
            <div id="pdf-sect-terms" className="p-6 bg-white border-t"
                style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
