  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">בניית הצעת מחיר</h1>
          <p className="text-slate-500 mt-1">בחר פריטים למחירון: {quote?.title}</p>
        </div>
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="space-y-3 max-h-64 overflow-y-auto">
              {!quoteLines || quoteLines.length === 0 ? (
                ...
                quoteLines.map(line => (
                  <div key={line.id} className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-slate-800">{line.name_snapshot}</p>
                      <p className="text-xs text-slate-500">כמות: {line.quantity}</p>
                    </div>
                    <Button 
                      size="sm" 
                      variant="ghost" 
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-12">
              <p className="text-slate-500">לא נמצאו פריטים בהתאם לסינון</p>
              <p className="text-sm text-slate-400 mt-2">נסה לשנות את הקריטריונים או לבטל חלק מהסינונים</p>
            </div>
          )}
        </div>
      </div>
